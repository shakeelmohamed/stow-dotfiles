...
suite("Configure (Add Docker files to Workspace)", function () {
    ...
    async function testDotNetCoreConsole(os, projectFolder, projectFileName, projectFileContents, expectedDockerFileContents) {
        ...
        }, [os /* it doesn't ask for a port, so we don't specify one here */], ['Dockerfile', '.dockerignore', `${projectFolder}/Program.cs`, `${projectFolder}/${projectFileName}`]);
    ...
    suite(".NET Core General", () => {
        global_test_1.testInEmptyFolder("No project file", async () => {
            await assertEx.throwsOrRejectsAsync(async () => testConfigureDocker('.NET Core Console', {
                ...
                packageFileType: undefined,
                packageFileSubfolderDepth: undefined
            }, ['Windows']), { message: "No .csproj file could be found. You need a C# project file in the workspace to generate Docker files for the selected platform." });
        });
        global_test_1.testInEmptyFolder("ASP.NET Core no project file", async () => {
            await assertEx.throwsOrRejectsAsync(async () => testConfigureDocker('ASP.NET Core', {}, ['Windows', '1234']), { message: "No .csproj file could be found. You need a C# project file in the workspace to generate Docker files for the selected platform." });
        });
        global_test_1.testInEmptyFolder("Multiple project files", async () => {
            ...
            await testConfigureDocker('.NET Core Console', {
                ...
                packageFileType: '.csproj',
                packageFileSubfolderDepth: '1'
            }, ['Windows', 'projectFolder2/aspnetapp.csproj'], ['Dockerfile', '.dockerignore', 'projectFolder1/aspnetapp.csproj', 'projectFolder2/aspnetapp.csproj']);
    ...
    suite("ASP.NET Core 2.2", async () => {
        global_test_1.testInEmptyFolder("Default port (80)", async () => {
            await writeFile('projectFolder1', 'aspnetapp.csproj', dotNetCoreConsole_21_ProjectFileContents);
            await testConfigureDocker('ASP.NET Core', undefined, ['Windows', undefined]);
            assertFileContains('Dockerfile', 'EXPOSE 80');
        });
        global_test_1.testInEmptyFolder("No port", async () => {
            await writeFile('projectFolder1', 'aspnetapp.csproj', dotNetCoreConsole_21_ProjectFileContents);
            await testConfigureDocker('ASP.NET Core', undefined, ['Windows', '']);
            assertNotFileContains('Dockerfile', 'EXPOSE');
        });
    ...
    suite("Java", () => {
        global_test_1.testInEmptyFolder("No port", async () => {
            await testConfigureDocker('Java', undefined, [''], ['Dockerfile', 'docker-compose.debug.yml', 'docker-compose.yml', '.dockerignore']);
            assertNotFileContains('Dockerfile', 'EXPOSE');
        });
        global_test_1.testInEmptyFolder("Default port", async () => {
            await testConfigureDocker('Java', undefined, [undefined], ['Dockerfile', 'docker-compose.debug.yml', 'docker-compose.yml', '.dockerignore']);
