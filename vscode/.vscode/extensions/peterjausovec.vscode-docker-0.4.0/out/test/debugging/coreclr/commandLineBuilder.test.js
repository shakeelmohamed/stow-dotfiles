...
const commandLineBuilder_1 = require("../../../debugging/coreclr/commandLineBuilder");
suite('debugging/coreclr/CommandLineBuilder', () => {
    function testBuilder(name, builderInitializer, expected, message) {
        test(name, () => {
            let builder = commandLineBuilder_1.default.create();
            builder = builderInitializer(builder);
            assert.equal(builder.build(), expected, message);
        });
    }
    suite('create', () => {
        test('No args', () => assert.equal(commandLineBuilder_1.default.create().build(), '', 'No arguments should return an empty command line.'));
        test('With args', () => assert.equal(commandLineBuilder_1.default.create('--arg1', '--arg2').build(), '--arg1 --arg2', 'The command line should contain the arguments.'));
        test('With factory', () => assert.equal(commandLineBuilder_1.default.create(() => '--arg1').build(), '--arg1', 'The command line should contain the argument.'));
        test('With undefined', () => assert.equal(commandLineBuilder_1.default.create(undefined).build(), '', 'No arguments should return an empty command line.'));
    });
    suite('withArg', () => {
...
//# sourceMappingURL=commandLineBuilder.test.js.map